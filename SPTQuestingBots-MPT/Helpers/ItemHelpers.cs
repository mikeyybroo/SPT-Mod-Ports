...
namespace SPTQuestingBots.Helpers
{
    public static class ItemHelpers
    {
        ...
        public static bool TryTransferItem(this BotOwner botOwner, Item item)
        {
            try
            {
                InventoryControllerClass inventoryController = GetInventoryController(botOwner);
                ...
                GStruct413<GClass2778> moveResult = GClass2770.Add(item, locationForItem, inventoryController, true);
        ...
        public static ItemAddress FindLocationForItem(this BotOwner botOwner, Item item, IEnumerable<EquipmentSlot> possibleSlots, InventoryControllerClass botInventoryController)
        {
            foreach (EquipmentSlot slot in possibleSlots)
            {
                ...
                foreach (GClass2495 grid in (equipmentSlot?.Grids ?? (new GClass2495[0])))
                {
                    ...
                    if (locationInGrid != null)
                    {
                        ...
                        return new GClass2765(grid, locationInGrid);
                    }
                }
            }
        ...
        public static DependencyGraph<IEasyBundle>.GClass3388 LoadBundle(this Item item)
        {
            try
            {
                if (item == null)

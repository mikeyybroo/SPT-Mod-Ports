...
namespace SPTQuestingBots.Controllers.Bots
{
    public class BotGenerator : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            float timeSinceSpawning = GClass1473.PastTimeSeconds(Singleton<AbstractGame>.Instance.GameTimer);
        ...
        private async Task generateBots(BotSpawner botSpawnerClass, BotDifficulty botdifficulty, int totalCount)
        {
            ...
            try
            {
                ...
                while (botsGenerated < totalCount)
                {
                    ...
                    try
                    {
                        GClass589 botProfileData = new GClass589(spawnSide, spawnType, botdifficulty, 0f, null);
                        GClass588 botSpawnData = await GClass588.Create(botProfileData, ibotCreator, botsInGroup, botSpawnerClass);

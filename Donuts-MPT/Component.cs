...
namespace Donuts
{
    public class DonutComponent : MonoBehaviour
    {
        ...
        private string runWeightedScenarioSelection()
        {
            ...
            /* TODO: Disable as we dont do scav raids
        ...
        private async Task SpawnBots(HotspotTimer hotspotTimer, Vector3 coordinate)
        {
            ...
            if (DonutsPlugin.hardStopOptionPMC.Value && (hotspotSpawnType == "pmc" || hotspotSpawnType == "sptusec" || hotspotSpawnType == "sptbear"))
            {
                ...
                var raidTimeLeft = EscapeTimeSeconds();
        ...
        public float EscapeTimeSeconds()
        {
            var GameTime = Singleton<AbstractGame>.Instance?.GameTimer;
            if (GameTime?.StartDateTime != null && GameTime?.EscapeDateTime != null)
            {
                DateTime? escapeDateTime = GameTime.EscapeDateTime;
                return (float)((escapeDateTime != null) ? (escapeDateTime.Value - DateTime.UtcNow) : TimeSpan.MaxValue)
                    .TotalSeconds;
            }

...
namespace DrakiaXYZ.QuestTracker
{
    ...
    public class QuestTrackerPlugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            _questClassField = AccessTools.GetDeclaredFields(notesTaskType).Single(x => x.FieldType == typeof(Quest));
        ...
        public void Update()
        {
            if (TasksScreenActive && Input.GetMouseButtonDown(1))
            {
                ...
                if (notesTask != null)
                {
                    var quest = _questClassField.GetValue(notesTask) as Quest;
            ...
            if (QuestsScreenActive && Input.GetMouseButtonDown(1))
            {
                ...
                {
                    Quest quest = questListItem.Quest;
                    HandleQuestRightClick(quest);
        ...
        private void HandleQuestRightClick(Quest quest)
        {
    ...
    class NotesTaskStatusPatch : ModulePatch
    {
        ...
        {
            var quest = _questClassField.GetValue(__instance) as Quest;
            if (QuestsTracker.IsTracked(quest))
            {
    ...
    class TasksScreenStatusComparePatch : ModulePatch
    {
        protected override MethodBase GetTargetMethod()
        {
            Type questStatusComparerType = StayInTarkovHelperConstants.EftTypes.First(x => x.Name == "QuestStatusComparer");

            return AccessTools.Method(questStatusComparerType, "Compare");
        }

        [PatchPostfix]
        public static void PatchPostfix(Quest x, Quest y, ref int __result)
        {
